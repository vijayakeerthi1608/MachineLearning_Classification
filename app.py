...
with st.sidebar:
    ...
    submit_btn = st.button("Submit / Predict", type="primary", use_container_width=True)

# Manage state clear if dataset is removed
if uploaded_file is None:
    st.session_state.pop("eval_results", None)
    st.info("Please upload a test CSV dataset and click 'Submit / Predict' to generate the dashboard metrics.")
    st.stop()

# Trigger predictions on Submit
if submit_btn:
    try:
...
# Render content if predictions exist in session_state
if "eval_results" in st.session_state:
    ...
    with tab2:
        st.dataframe(
            comparison_df.style.highlight_max(axis=0, color="#1b4332" if is_dark else "#d8f3dc"),
    ...
    st.info("Click 'Submit / Predict' to evaluate models on the uploaded file.")